 ...
 * 找到当前Fiber的root
 * 给更新节点的父节点链上的每个节点的expirationTime设置为这个update的expirationTime 除非它本身时间小于expirationTime
 * 给更新节点的父节点链上的每个节点的childExpirationTime设置为这个update的expirationTime 除非它本身时间要小于expirationTime
 * 最后返回root节点的Fiber对象
 */
function scheduleWork(fiber: Fiber, expirationTime: ExpirationTime) {
  ...
  /**
   * isWorking 是否正在工作 在renderRoot和commitRoot的时候设置为true 也就是render和commit两个阶段都为true
   * nextRenderExpirationTime 在是新的renderRoot的时候会被设置为当前任务的expirationTime 一旦被改 只有当下次任务是Nowork的时候才会被设置为noWork
   * 
   */
  if (
    !isWorking &&
